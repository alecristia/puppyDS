...
```{r fig-all-error}
p <- ggplot(subset(overall_means_nogroup), aes(y = f1, x = f2, col = factor(ds)))+
  ...
  annotate("text", y= 350,x=2800,label="i") +
  annotate("text", y= 850,x=1500,label="a") +
  annotate("text", y= 350,x=1350,label="u") +
      geom_errorbarh(aes(xmin = overall_means_nogroup$f2-overall_sds_nogroup$f2/sqrt(10),xmax = overall_means_nogroup$f2+overall_sds_nogroup$f2/sqrt(10))) + 
    geom_errorbar(aes(ymin = overall_means_nogroup$f1-overall_sds_nogroup$f1/sqrt(10),ymax = overall_means_nogroup$f1+overall_sds_nogroup$f1/sqrt(10))) + theme(legend.title = element_blank()) +labs(y= "F2 (Hz)", x = "F1 (Hz)")
 
p

jpeg("output/fig-vss.jpg",res=300,width=7, height=5, units="cm")
p
dev.off()
