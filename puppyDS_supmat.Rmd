---
title: 'Supplementary materials: Hyperarticulation in Infant-Directed Speech as a Consequence  of Vocal Emotion'
output:
  pdf_document:
    toc: yes
    toc_depth: 3
...
p <- ggplot(subset(overall_means_nogroup), aes(x = f1, y = f2, col = factor(ds)))+
  ...
  annotate("text", x= 850,y=1350,label="a") +
  annotate("text", x= 350,y=1350,label="u") +
      geom_errorbar(aes(ymin = overall_means_nogroup$f2-overall_sds_nogroup$f2/sqrt(10),ymax = overall_means_nogroup$f2+overall_sds_nogroup$f2/sqrt(10))) + 
    geom_errorbarh(aes(xmin = overall_means_nogroup$f1-overall_sds_nogroup$f1/sqrt(10),xmax = overall_means_nogroup$f1+overall_sds_nogroup$f1/sqrt(10))) + theme(legend.title = element_blank()) +labs(y= "F2 (Hz)", x = "F1 (Hz)")
