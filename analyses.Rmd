...
- "REAL" decisions:
  - removing outliers
  - removing items from f1/2 when no f0
  - doing ANOVA first, then mixed model
  - (I don't think software matters!)
...
#dat[is.na(dat$Word),] #most NA lines are totally empty -- remove
...
dat %>%
    group_by(ds,v,stim_group) %>%
    dplyr::summarize(f1 = mean(f1, na.rm=TRUE),
                     f2 = mean(f2, na.rm=TRUE)) -> direct_overall_means
...
p <- ggplot(subset(overall_means,stim_group=="box"), aes(x = f1, y = f2, col = factor(ds), fill = factor(ds)))+
  ...
  geom_point() +ggtitle("Box - average first within moms")+ scale_fill_manual(values=group.colors)
p
```

Try again, now by using overall means that do not average first within mothers!
...
p <- ggplot(subset(direct_overall_means,stim_group=="box"), aes(x = f1, y = f2, col = factor(ds), fill = factor(ds)))+
  geom_polygon(data = subset(direct_overall_means,stim_group=="box") ,
  aes(
     x=f1 ,
    y=f2 ,
    group=ds
  ), fill=NA) + #alpha = 0.3 (if we want fill
  geom_point() +ggtitle("Box - average across all tokens")+ scale_fill_manual(values=group.colors)
p
```
...
p <- ggplot(subset(overall_means,stim_group=="ball"), aes(x = f1, y = f2, col = factor(ds)))+
  ...
  geom_point() +ggtitle("Ball - averaging first within moms") 
p

```

```{r fig2-direct}
p <- ggplot(subset(direct_overall_means,stim_group=="ball"), aes(x = f1, y = f2, col = factor(ds)))+
  geom_polygon(data = subset(direct_overall_means,stim_group=="ball") ,
  aes(
     x=f1 ,
    y=f2 ,
    group=ds
  ), fill=NA) + #alpha = 0.3 (if we want fill)
  geom_point() +ggtitle("Ball - averaging across all tokens") 
p
...
pivot_wider(vs,names_from="ds",values_from="vss")->vs_long
write.csv(vs_long,"vs_long.csv",row.names=F)
